...
| 命令 | 描述 |
|------|------|
| `npm run build` | 生产环境构建（压缩代码，生成sourcemap） |
| `npm run dev` | 开发模式（文件监听，自动重构） |
| `npm run build:watch` | 构建监听模式（等同于dev） |
| `npm run clean` | 清理dist构建目录 |
| `npm run type-check` | TypeScript类型检查 |
| `npm run lint` | ESLint代码检查 |
| `npm run package` | 清理+构建+打包为zip文件 |
| `npm run zip` | 仅将dist目录压缩为zip |
